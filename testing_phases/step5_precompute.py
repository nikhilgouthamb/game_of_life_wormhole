...
def get_diagonal_neighbor(r: int, c: int, dr: int, dc: int) -> Optional[tuple[int,int]]:
    ...
    return final
# ─────────────────────────────────────────────────────────────────────────────
# Step 5: Precompute the 8‐neighbor list for every cell
# ─────────────────────────────────────────────────────────────────────────────

# (Assumes H, W, vertical_map, horizontal_map,
#  get_cardinal_neighbor, and get_diagonal_neighbor are already defined.)

# (1) Define the eight direction offsets
CARDINAL_OFFSETS = [(-1,  0),  # up
                    (+1,  0),  # down
                    ( 0, -1),  # left
                    ( 0, +1)]  # right

DIAGONAL_OFFSETS = [(-1, -1),  # up‐left
                    (-1, +1),  # up‐right
                    (+1, -1),  # down‐left
                    (+1, +1)]  # down‐right

# (2) Allocate an H×W list of lists
neighbors: list[list[tuple[int,int]]] = [[[] for _ in range(W)] for _ in range(H)]

# (3) Loop over every cell (r,c)
for r in range(H):
    for c in range(W):
        # (3a) Four cardinal neighbors
        for (dr, dc) in CARDINAL_OFFSETS:
            nbr = get_cardinal_neighbor(r, c, dr, dc)
            if nbr is not None:
                neighbors[r][c].append(nbr)

        # (3b) Four diagonal neighbors
        for (dr, dc) in DIAGONAL_OFFSETS:
            nbr = get_diagonal_neighbor(r, c, dr, dc)
            if nbr is not None:
                neighbors[r][c].append(nbr)

# At this point, `neighbors[r][c]` is a list of 0–8 tuples (r2,c2),
# each representing a valid live/dead‐count neighbor under the tunnel rules.
#
# Example: print the neighbors of cell (r=2,c=3):
# print("Neighbors of (2,3):", neighbors[2][3])